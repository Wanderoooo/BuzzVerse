        ...
        <Theme>
            <Flex direction='column' gap='3' m='3'>
                <Avatar.Root>
                    ...
                    onClick={props.clicks[0]} key='friendsserver'
                    />
                </Avatar.Root>
                <Separator orientation="horizontal" size='3'/>
                {props.serverList.map((server) => 
                    <Avatar.Root key={'server' + server.name}>
                        <Avatar.Image 
                        width='65px' src={server.icon} alt={server.name} onClick={props.clicks[props.serverList.indexOf(server)]} 
                        />
                    </Avatar.Root>
                )}
            </Flex>
